...
const initSlideshow = () => {
  ...
  // Perform initial state health check and repair if needed
  const healthCheck = stateValidator.performHealthCheck();
  if (!healthCheck.healthy) {
    console.log('🔧 State inconsistencies detected, attempting automatic repair...');
    const repairCount = stateValidator.repairInconsistentState();
    if (repairCount > 0) {
      console.log(`✅ Repaired ${repairCount} state issues, re-running health check...`);
      stateValidator.performHealthCheck();
    }
  }
  ...
  setInterval(() => {
    const healthCheck = stateValidator.performHealthCheck();
    if (!healthCheck.healthy && healthCheck.errors.length > 0) {
      console.log('⚠️ Periodic health check failed, attempting repair...');
      stateValidator.repairInconsistentState();
    }
  }, 5 * 60 * 1000); // 5 minutes
};
  ...
  publishConfig("number", config)
    .on("message", async (topic, message) => {
      if (topic === config.command_topic) {
        ...
        // Use atomic transaction to ensure both persistence and runtime update succeed
        const success = await atomicSettingUpdate(
          "slideshow_interval",
          interval,
        ...
        if (!success) {
          console.error("Failed to update slideshow interval atomically");
          // Could publish error state to MQTT here
        }
      }
    })
    .subscribe(config.command_topic);
...
/**
 * Atomic transaction system for setting updates
 * Ensures both persistence and runtime config updates succeed or both rollback
 */
const atomicSettingUpdate = async (key, value, configUpdate = null) => {
  // Create backup of current state
  const argsFilePath = path.join(require("os").homedir(), ".config", "touchkio", "Arguments.json");
  const backupPath = `${argsFilePath}.transaction_backup`;

  let originalValue = null;
  let originalConfigValue = null;

  try {
    // Backup current state
    if (fs.existsSync(argsFilePath)) {
      fs.copyFileSync(argsFilePath, backupPath);
      originalValue = ARGS[key];
    }

    // If there's a config update, backup original config value
    if (configUpdate && typeof configUpdate === 'object') {
      const configKey = Object.keys(configUpdate)[0];
      const status = slideshow.getStatus();
      originalConfigValue = status.config[configKey];
    }

    // Validate setting before applying
    const validation = stateValidator.validateSetting(key, value);
    if (!validation.valid) {
      throw new Error(`Invalid setting value: ${validation.errors.join(', ')}`);
    }

    // Perform atomic transaction
    console.log(`🔄 Starting atomic update: ${key} = ${value}`);

    // Step 1: Update Arguments.json
    updateSlideshowSetting(key, value);

    // Step 2: Update runtime config if provided
    if (configUpdate) {
      slideshow.updateConfig(configUpdate);
    }

    // Step 3: Sync to runtime (this also validates the update worked)
    updateSlideshowRuntimeConfig(key, value);

    console.log(`✅ Atomic update completed: ${key} = ${value}`);
    ...
    console.error(`❌ Atomic update failed for ${key}: ${error.message}`);

    // CRITICAL: Rollback all changes
    try {
      // Restore Arguments.json from backup
      if (fs.existsSync(backupPath)) {
        fs.copyFileSync(backupPath, argsFilePath);
        console.log("🔄 Restored Arguments.json from backup");

        // Reload ARGS from restored file
        if (originalValue !== null) {
          ARGS[key] = originalValue;
        }
      }

      // Restore runtime config if it was changed
      if (configUpdate && originalConfigValue !== undefined) {
        const configKey = Object.keys(configUpdate)[0];
        slideshow.updateConfig({ [configKey]: originalConfigValue });
        console.log(`🔄 Restored runtime config: ${configKey} = ${originalConfigValue}`);
      }
...
/**
 * Centralized state validator and consistency checker
 * Validates state across ARGS, runtime config, and MQTT
 */
class SlideshowStateValidator {
  constructor() {
    this.validationRules = {
      // Slideshow behavior
      slideshow_enabled: { type: 'boolean', required: true },
      slideshow_active: { type: 'boolean', required: true },
      slideshow_interval: { type: 'number', min: 1, max: 3600, required: true },
      slideshow_idle_timeout: { type: 'number', min: 1, max: 180, required: true },

      // Element visibility
      slideshow_show_clock: { type: 'boolean', required: true },
      slideshow_show_date: { type: 'boolean', required: true },
      slideshow_show_source: { type: 'boolean', required: true },
      slideshow_show_counter: { type: 'boolean', required: true },
      slideshow_show_metadata: { type: 'boolean', required: true },

      // Positioning
      slideshow_clock_position: { type: 'string', enum: ['top-left', 'top-center', 'top-right', 'center-left', 'center', 'center-right', 'bottom-left', 'bottom-center', 'bottom-right', 'custom'], required: true },

      // Styling
      slideshow_clock_opacity: { type: 'number', min: 0, max: 100, required: true },
      slideshow_clock_background_opacity: { type: 'number', min: 0, max: 100, required: true },

      // Advanced
      slideshow_animation_enabled: { type: 'boolean', required: true },
      slideshow_animation_speed: { type: 'number', min: 0.1, max: 5.0, required: true },
      preset_editor_mode: { type: 'boolean', required: true }
    };
  }
  ...
  validateSetting(key, value) {
    ...
    if (!rule) {
      console.warn(`⚠️ No validation rule for setting: ${key}`);
      return { valid: true, warnings: [`No validation rule for ${key}`] };
    }
    ...
    if (rule.type === 'number') {
      ...
      if (isNaN(num)) {
        errors.push(`${key} must be number, got: ${value}`);
      } else {
        if (rule.min !== undefined && num < rule.min) {
          errors.push(`${key} must be >= ${rule.min}, got: ${num}`);
        }
        if (rule.max !== undefined && num > rule.max) {
          errors.push(`${key} must be <= ${rule.max}, got: ${num}`);
        }
      }
    }
  ...
  /**
   * Validates consistency between ARGS, runtime config, and actual slideshow state
   */
  validateStateConsistency() {
    ...
    try {
      ...
      const slideshowActive = status.active;

      // Check critical consistency issues

      // 1. slideshow_active vs actual state
      const argsActive = ARGS.slideshow_active === 'true' || ARGS.slideshow_active === true;
      if (argsActive !== slideshowActive) {
        issues.push({
          severity: 'error',
          message: `slideshow_active inconsistent: ARGS=${argsActive}, runtime=${slideshowActive}`
        });
      }

      // 2. Editor mode consistency
      const argsEditorMode = ARGS.preset_editor_mode === 'true' || ARGS.preset_editor_mode === true;
      const runtimeEditorMode = runtimeConfig.editorMode === true;
      if (argsEditorMode !== runtimeEditorMode) {
        issues.push({
          severity: 'warning',
          message: `Editor mode inconsistent: ARGS=${argsEditorMode}, runtime=${runtimeEditorMode}`
        });
      }

      // 3. Interval consistency
      const argsInterval = parseInt(ARGS.slideshow_interval) * 1000;
      const runtimeInterval = runtimeConfig.interval;
      if (Math.abs(argsInterval - runtimeInterval) > 100) { // Allow 100ms tolerance
        issues.push({
          severity: 'warning',
          message: `Interval inconsistent: ARGS=${argsInterval}ms, runtime=${runtimeInterval}ms`
        });
      }

      // 4. Required settings validation
      Object.entries(this.validationRules).forEach(([key, rule]) => {
        if (rule.required && ARGS[key] === undefined) {
          issues.push({
            severity: 'error',
            message: `Required setting missing: ${key}`
          });
        }
      });
  ...
  /**
   * Performs comprehensive state health check
   */
  performHealthCheck() {
    console.log('🔍 Performing slideshow state health check...');

    const issues = this.validateStateConsistency();
    const errors = issues.filter(i => i.severity === 'error');
    const warnings = issues.filter(i => i.severity === 'warning');

    if (errors.length > 0) {
      console.error('❌ State health check FAILED:');
      errors.forEach(error => console.error(`  • ${error.message}`));
    }

    if (warnings.length > 0) {
      console.warn('⚠️ State health check warnings:');
      warnings.forEach(warning => console.warn(`  • ${warning.message}`));
    }

    if (errors.length === 0 && warnings.length === 0) {
      console.log('✅ State health check PASSED - all systems consistent');
    }
  ...
  repairInconsistentState() {
    console.log('🔧 Attempting to repair inconsistent state...');
    ...
    errors.forEach(error => {
      try {
        if (error.message.includes('slideshow_active inconsistent')) {
          // Sync ARGS to actual slideshow state (runtime is authoritative)
          const status = slideshow.getStatus();
          updateSlideshowSetting('slideshow_active', status.active);
          console.log(`🔧 Repaired slideshow_active: synced to runtime state (${status.active})`);
          repairCount++;
        }

        if (error.message.includes('Required setting missing')) {
          const settingMatch = error.message.match(/Required setting missing: (\w+)/);
          if (settingMatch) {
            const setting = settingMatch[1];
            const rule = this.validationRules[setting];

            // Set to default value based on type
            let defaultValue;
            if (rule.type === 'boolean') defaultValue = false;
            else if (rule.type === 'number') defaultValue = rule.min || 0;
            else if (rule.type === 'string' && rule.enum) defaultValue = rule.enum[0];
            else defaultValue = '';

            updateSlideshowSetting(setting, defaultValue);
            console.log(`🔧 Repaired missing setting: ${setting} = ${defaultValue}`);
            repairCount++;
          }
        }

      } catch (repairError) {
        console.error(`❌ Failed to repair: ${error.message} - ${repairError.message}`);
      }
    });

    console.log(`✅ State repair completed: fixed ${repairCount} issues`);
    return repairCount;
  }
}

// Global state validator instance
const stateValidator = new SlideshowStateValidator();
  ...
  try {
    switch (key) {
      ...
      case "slideshow_clock_border_radius":
        ...
        break;
      case "slideshow_date_border_radius":
        slideshow.updateConfig({ dateBorderRadius: value });
        break;
      case "slideshow_source_border_radius":
        slideshow.updateConfig({ sourceBorderRadius: value });
        break;
      case "slideshow_counter_border_radius":
        slideshow.updateConfig({ counterBorderRadius: value });
        break;
      case "slideshow_metadata_border_radius":
        slideshow.updateConfig({ metadataBorderRadius: value });
        break;
      // Padding settings for all elements
      case "slideshow_clock_padding":
        slideshow.updateConfig({ clockPadding: value });
        break;
      case "slideshow_date_padding":
        slideshow.updateConfig({ datePadding: value });
        break;
      case "slideshow_source_padding":
        slideshow.updateConfig({ sourcePadding: value });
        break;
      case "slideshow_counter_padding":
        slideshow.updateConfig({ counterPadding: value });
        break;
      case "slideshow_metadata_padding":
        slideshow.updateConfig({ metadataPadding: value });
        break;
      // Shadow settings for all elements
      case "slideshow_clock_shadow":
        slideshow.updateConfig({ clockShadow: value });
        break;
      case "slideshow_date_shadow":
        slideshow.updateConfig({ dateShadow: value });
        break;
      case "slideshow_source_shadow":
        slideshow.updateConfig({ sourceShadow: value });
        break;
      case "slideshow_counter_shadow":
        slideshow.updateConfig({ counterShadow: value });
        break;
      case "slideshow_metadata_shadow":
        slideshow.updateConfig({ metadataShadow: value });
        break;
      // Custom position coordinates
      case "slideshow_clock_custom_x":
        slideshow.updateConfig({ clockCustomX: value });
        break;
      case "slideshow_clock_custom_y":
        slideshow.updateConfig({ clockCustomY: value });
        break;
      case "slideshow_date_custom_x":
        slideshow.updateConfig({ dateCustomX: value });
        break;
      case "slideshow_date_custom_y":
        slideshow.updateConfig({ dateCustomY: value });
        break;
      case "slideshow_source_custom_x":
        slideshow.updateConfig({ sourceCustomX: value });
        break;
      case "slideshow_source_custom_y":
        slideshow.updateConfig({ sourceCustomY: value });
        break;
      case "slideshow_counter_custom_x":
        slideshow.updateConfig({ counterCustomX: value });
        break;
      case "slideshow_counter_custom_y":
        slideshow.updateConfig({ counterCustomY: value });
        break;
      case "slideshow_metadata_custom_x":
        slideshow.updateConfig({ metadataCustomX: value });
        break;
      case "slideshow_metadata_custom_y":
        slideshow.updateConfig({ metadataCustomY: value });
        break;
